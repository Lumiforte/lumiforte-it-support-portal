-- Update ticket activity trigger to also log submitter changes
...
BEGIN
  ...
  -- Log submitter changes
  IF (TG_OP = 'UPDATE' AND OLD.created_by != NEW.created_by) THEN
    INSERT INTO public.ticket_activities (ticket_id, user_id, action_type, old_value, new_value)
    VALUES (NEW.id, auth.uid(), 'submitter_changed', OLD.created_by::text, NEW.created_by::text);
  END IF;
