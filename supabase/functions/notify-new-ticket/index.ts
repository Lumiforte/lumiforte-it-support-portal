...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { ticketId, title, description, priority, category, userName, userEmail }: NotifyTicketRequest = await req.json();
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <p><strong>Categorie:</strong> ${category || "Niet gespecificeerd"}</p>
            <p><strong>Prioriteit:</strong> <span style="color: ${priorityColor}; font-weight: bold;">${priority.toUpperCase()}</span></p>
          </div>
